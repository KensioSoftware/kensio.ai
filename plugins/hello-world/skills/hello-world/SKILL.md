...
A placeholder skill whose only job is to prove the install path works end to end: marketplace →
plugin → skill → loaded into the session.
...
2. Which install channel they appear to be using, if it is obvious from the conversation
   (marketplace, npm package, or downloaded archive).
